...
class CopyPasteIncrementInputCommand(sublime_plugin.TextCommand):

	def run(self, edit):		
		# Gather Input
		window = self.view.window()
		window.show_input_panel("Number of duplications:", "1", lambda x: self.view.run_command('copy_paste_increment', {"num_duplications": x}), None, None)

# Duplicates and increments the selected content
class CopyPasteIncrementCommand(sublime_plugin.TextCommand):
	...
	# one as a string. The 'increment' in CPI.
	def func(self, m):
		n = int(m.group())
		return str(n + 1)
